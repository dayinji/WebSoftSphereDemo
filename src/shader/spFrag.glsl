...
void main() {
	...
	// 紊乱法线制造磨砂材质效果
	// Turbulent normal to create frosted material effect
	if (frosted == 1.0)
		n = normalize(n+0.15*snoiseVec3(n*vec3(40.0)));
	...
	// fresnelVal：菲涅尔值
	float fresnelVal = pow(dot(n, E), 0.8);
	...
	if (bgType == 0.0) {
		...
		bgColor = mix(bgColor1, bgColor2, 1.0 - texUv.y);
	} else if (bgType == 2.0) {
		bgColor = mix(bgColor1, bgColor2, dot(texUv - vec2(0.0, 1.0), vec2(1.0, -1.0) ));
	} else if (bgType == 3.0) {
		bgColor = mix(bgColor1, bgColor2, length(texUv-vec2(0.8)));
	}
	// diffuseColor：加了菲涅尔效果的着色
	vec3 diffuseColor = mix(bgColor, vec3(0.08), fresnelVal);
